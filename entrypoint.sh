...
if [ "$OVPN_IPV6_ENABLE" = "true" ]; then
  ...
  export OVPN_IPV6_PUSH_INTERNAL_SUBNET1="push \"route-ipv6 $OVPN_IPV6_INTERNAL_NETWORK1\""
  export OVPN_IPV6_PUSH_INTERNAL_SUBNET2="push \"route-ipv6 $OVPN_IPV6_INTERNAL_NETWORK2\""
  export OVPN_IPV6_PUSH_INTERNAL_SUBNET3="push \"route-ipv6 $OVPN_IPV6_INTERNAL_NETWORK3\""
else
  export OVPN_IPV6_CONFIG=""
  export OVPN_IPV6_ROUTE=""
  export OVPN_IPV6_DNS=""
  export OVPN_IPV6_PUSH_SUBNET=""
  export OVPN_IPV6_PUSH_INTERNAL_SUBNET1=""
  export OVPN_IPV6_PUSH_INTERNAL_SUBNET2=""
  export OVPN_IPV6_PUSH_INTERNAL_SUBNET3=""
fi

# 渲染OpenVPN配置（使用白名单变量）
envsubst '$OVPN_PORT $OVPN_PROTO $OVPN_DEV $OVPN_CA_CERT $OVPN_SERVER_CERT $OVPN_SERVER_KEY $OVPN_DH_PEM $OVPN_NETWORK $OVPN_NETMASK $OVPN_DNS_IPV4 $OVPN_IPV6_CONFIG $OVPN_IPV6_ROUTE $OVPN_IPV6_DNS $OVPN_CIPHER $OVPN_IPV6_PUSH_SUBNET $OVPN_IPV6_PUSH_INTERNAL_SUBNET1 $OVPN_IPV6_PUSH_INTERNAL_SUBNET2 $OVPN_IPV6_PUSH_INTERNAL_SUBNET3'  < /etc/openvpn/server.conf.template > /etc/openvpn/server.conf
